...
starty <- 2011
endy <- 2022
vars <- c("Número de Cotizantes")
sectors <- c("A - AGRICULTURA, GANADERÍA, CAZA, SILVICULTURA Y PESCA",
...
data2 <- data %>% 
  group_by(Country, Departamento, Municipio, year, variable) %>% 
  summarize(value = sum(value, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(sector = "All sectors")
data$value <- ifelse(is.na(data$value), 0, data$value)

out <- rbind(data, data2) %>% 
  as.data.frame() %>% 
  dplyr::arrange(Country)

write.csv(out, paste("pila", Sys.Date(), starty, endy, length(vars), "vars.csv", sep = "_"), row.names = F)

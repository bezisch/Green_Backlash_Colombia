...
for ( i in 1:length(resc.var)){
  ...
  #mean for each distyear --> not corrected, do it later 
  ...
  prop <- tapply(seq(along=clea.m$lowe_environment), clea.m$distyear, 
...
#  maxvote <- data.frame(names(maxvote), maxvote)
 ...
 # names(maxvote) <- c("distyear", paste0("maxvote", resc.var[i]))
  
#  clea.m <- merge(clea.m, maxvote, by.x="distyear", by.y=1, all.x=TRUE)
  
#  clea.m$green <-temp==clea.m[,paste0("maxvote", resc.var[i])]
  
  clea.m$green[is.na(clea.m$green)] <- FALSE
  
  #vote share of green party for each distyear 
  green <- tapply(seq(along=temp), clea.m$distyear, 
  ...
  green.check <- tapply(seq(along=temp), clea.m$distyear, 
                      function(i, x=clea.m$green) sum(x[i], na.rm=TRUE))
  
  
  if(is.na(mean(green.check))){cat("problem here!")}
  
  mv <- data.frame(names(median), median, mean, mean_raw, prop) # we do not consider maxvote here, not needed at the moment.
  
  names(mv) <- c("distyear", paste0(c("MV", "MEAN", "MEAN_raw", "PROP"), resc.var[i]))
...
mean_dataframe <- as.data.frame(mean)
median_dataframe <- as.data.frame(median)



summary(summaries.mv.district$MEANenvironment)
summary(summaries.mv.district$MEANleftright)

summary(summaries.mv.district$MVenvironment)
summary(summaries.mv.district$MVleftright)
